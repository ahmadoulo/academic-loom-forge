...
serve(async (req) => {
  ...
  try {
    ...
    if (teacherError) {
      ...
      return new Response(
        JSON.stringify({
          ...
          message: "Erreur lors de la vérification du professeur",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
    ...
    if (!teacher) {
      return new Response(
        JSON.stringify({
          ...
          message:
            "Aucun professeur trouvé avec cet email pour cette école. Contactez votre établissement.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
    ...
    if (!resendApiKey) {
      console.warn("verify-teacher-account: RESEND_API_KEY missing (test mode)");
      return new Response(
        JSON.stringify({
          success: true,
    ...
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #333;">Bienvenue ${teacher.firstname} ${teacher.lastname} !</h1>
        <p>Votre compte professeur est prêt pour l'établissement <strong>${school.name}</strong>.</p>
    ...
    if (!resendResponse.ok) {
      console.error("verify-teacher-account:resendError", resendData);
      return new Response(
        JSON.stringify({
          success: true,
