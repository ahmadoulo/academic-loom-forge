...
Deno.serve(async (req) => {
  ...
  try {
    const { sessionToken, attendanceId }: Body = await req.json();
    ...
    if (attendanceError || !attendance) {
      return new Response(JSON.stringify({ success: false, message: "Enregistrement introuvable" }), {
        status: 404,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    if (!isAdminForSchool(rolesRes.roles, attendance.school_id)) {
    ...
    const { error } = await supabaseAdmin.from("event_attendance").delete().eq("id", attendanceId);
    ...
    console.error("delete-event-attendance error:", error);
