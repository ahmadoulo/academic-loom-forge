...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (enrollmentError) {
      ...
      return new Response(
        JSON.stringify({
          ...
          message: "Erreur lors de la verification de l'etudiant",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
    ...
    if (!student) {
      return new Response(
        JSON.stringify({
          ...
          message:
            "Aucun etudiant trouve avec cet email pour cette ecole. Contactez votre etablissement.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
    ...
    if (!smtpHost || !smtpUsername || !smtpPassword || !smtpFromAddress) {
      console.warn("verify-student-account: SMTP not configured (test mode)");
      return new Response(
        JSON.stringify({
          success: true,
                ...
                <table role="presentation" style="width: 100%; max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05);">
                  ...
                  <tr>
                    <td style="padding: 32px 32px 0;">
                      <h1 style="margin: 0 0 16px; color: #1e293b; font-size: 24px; font-weight: 700; line-height: 1.3;">Bienvenue ${student.firstname} ${student.lastname} !</h1>
                      <p style="margin: 0 0 16px; color: #475569; font-size: 15px; line-height: 1.6;">
                        Votre compte etudiant est pret pour l'etablissement <strong>${school.name}</strong>.
                      </p>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td style="padding: 0 32px 32px; text-align: center;">
                      <a href="${invitationUrl}" 
                         style="display: inline-block; padding: 14px 32px; background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 15px; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3);">
                        Activer mon compte
                      </a>
                    </td>
                  </tr>
    ...
    try {
      await transporter.sendMail({
        ...
        to: normalizedEmail,
        subject: `Activez votre compte etudiant - ${school.name}`,
        html: emailHtml,
      });

      console.log("verify-student-account: Email sent successfully to:", normalizedEmail);
      ...
      console.error("verify-student-account:emailError", emailError);
      return new Response(
        JSON.stringify({
          success: true,
